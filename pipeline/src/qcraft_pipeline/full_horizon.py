...
def canonical(payload):
    """Sorted keys, ordered arrays, compact ASCII JSON, finite numbers."""
    return json.dumps(
...
def main(argv=None):
    ...
    if (
        out == base.resolve()
        or out == config.vintage_dir(config.BASE_VINTAGE_ID).resolve()
    ):
    ...
    macro = emit.normalise_non_finite(
        weo.build_macrofiscal(
            paths["weo"], paths["weo_countries"], names, year_max=release_end
        )
    )
    by_country = {}
    ...
    selected = [
        r
        for r in index["countries"]
        if not args.countries or r["iso3c"] in args.countries
    ]
    json_dir = out / "json"
    ...
    for entry in selected:
        ...
        if h["coverageStatus"] == "full" and h["weoMaxYear"] < release_end:
            ...
            h["coverageReason"] = (
                f"Country WEO inputs end at {h['weoMaxYear']}; "
                f"this release extends to {release_end} for other countries."
            )
        h = {
            "id": POLICY,
            "dataRevision": REVISION,
            "sourceVintage": config.VINTAGE_ID,
            **h,
    ...
    verified_dir = config.vintage_dir(config.BASE_VINTAGE_ID) / "json"
    verified_hashes = {
        p.stem: hashlib.sha256(canonical(json.loads(p.read_text()))).hexdigest()
        for p in sorted(verified_dir.glob("*.json"))
        if p.stem != "index"
    }
    index.update(
        {
            "verifiedInputHashes": verified_hashes,
            "dataRevision": REVISION,
            "calculationPolicy": POLICY,
            "releaseWeoMaxYear": release_end,
            "count": len(selected),
            "countries": selected,
        }
    )
    ...
    manifest.update(
        {
            ...
            "calculationPolicy": POLICY,
            "generated_utc": None,
            "rebuild": "python -m qcraft_pipeline.full_horizon (offline)",
            "macrofiscal_year_max": release_end,
            "macrofiscal_year_max_note": (
                "Complete cached WEO release; country usable horizon checked "
                "explicitly. Baseline assumptions and incremental climate "
                "comparisons begin H+1; climate calendar unchanged, anchored "
                "at H; no catch-up."
            ),
            "observationStatus": (
                "LATEST_ACTUAL_ANNUAL_DATA and PUBLICATION_DATE are blank in "
                "the cached SDMX rows. Source WEO values are "
                "estimates/projections; this release does not assert a common "
                "observed/forecast cutoff."
            ),
            "retainedInputs": (
                "Demography, productivity and climate arrays copied without "
                "numeric changes from each old Current JSON payload; old "
                "folders unchanged."
            ),
            "inputHashDefinition": (
                "SHA-256 of UTF-8 Python json.dumps(raw payload excluding "
                "horizonPolicy, sort_keys=True, separators=(',', ':'), "
                "allow_nan=False, ensure_ascii=True), no newline. Raw payload "
                "includes country identity and four ordered input arrays."
            ),
            "coverage": coverage,
        }
    )
    ...
    manifest["country_json"] = {
        "files": len(selected),
        "bytes": sum(p.stat().st_size for p in json_dir.glob("*.json")),
    }
    manifest["selectable_countries"] = len(selected)
    manifest["coveragePolicy"] = (
        "Use the contiguous complete 2009..H macro/fiscal window. "
        "Preserve later incomplete source rows. Missing history, WDI bridge or "
        "calendar climate index blocks the calculation; a shorter usable horizon "
        "is disclosed. Undefined effective rates on zero debt retain the legacy "
        "carry/zero treatment."
    )
        ...
        json.dumps(
            {
                ...
                "coverage": {
                    s: sum(r["coverageStatus"] == s for r in coverage)
                    for s in ("full", "shorter", "unsupported")
                },
            },
            indent=2,
        )
