...
## The question from above is about trouble, not about 2099
...
- **Derive** the debt dynamics equation in words, without notation, and check that the notation matches
...
The map below narrows to a single box.
...
::: {.callout-important}
## DRAFT FOR TEAL: building the equation from words
...
## Only one of the three terms is a decision

The figure below tints each term and gives it one plain English phrase, so the notation and the sentence sit on the same page.

{{< include figures/_m2-equation-annotated.qmd >}}

Read it left to right and the asymmetry is the thing to keep: two of the three terms arrive from outside the budget process, and only the last one is something a government sets this year.

::: {.callout-note collapse="true"}
## Prefer it in numbers? Three years of the debt dynamics equation

The same setup as @sec-m1: debt at 60 percent of GDP, the effective interest rate at 8 percent, nominal growth at 6 percent, a primary deficit of 1 percent of GDP held flat.

Each year does the same two operations. Multiply by 1.08 and divide by 1.06, which is a factor of 1.0189. Then add the 1 point of GDP the government borrowed.
...
The borrowing contributes exactly 1 point a year. Everything above that comes from the amplifier, and it grows each year, because it is applied to a bigger stock: 1.1 points in year one, 2.2 by year three.

**Sensitivity.** Run the identical three years with growth at 7 percent instead of 6 and the ratio lands at 64.7 rather than 66.5. One point of growth, held for three years, is worth 1.8 points of debt ratio. Held for seventy, it is worth a different fiscal policy.
:::

## The interest-growth gap moves the ratio with nobody borrowing

The next figure runs the equation forward ten times at three different interest-growth pairs, with the primary balance nailed to zero so nothing else can be responsible for the result.

{{< include figures/_m2-scoreboard.qmd >}}

Three percentage points of differential separate a country at 66 percent of GDP from one at 38, and no government in that picture made a single decision about spending.
...
**Sensitivity.** Take one point off growth, to 5 percent, and the required surplus rises from 1.4 to 1.9 percent of GDP. The bill for standing still went up by half a point of GDP because a macro variable moved, and nobody in the budget process decided anything.
...
The debt dynamics equation appears below as a diagram with three nodes left blank. Fill them in before opening the answer, because constructing the map is the exercise and reading a finished one is not.
...
**Setup.** Open [Q-CRAFT Explorer](https://tealinsights.shinyapps.io/q-craft_explorer1/), select a country you know well, fiscal rule **off**.
...
**Explain.** Reconcile the two. If the primary balance is in deficit and the ratio is not exploding, the scoreboard is working in that country's favour: nominal growth is beating the effective interest rate, which is the normal position for a country still borrowing largely on concessional terms. If your prediction assumed a deficit must mean rising debt, that is the misconception from @sec-m0 question 1, and now you have seen it fail on real data.
...
- You can build the debt dynamics equation from a sentence and check that the notation matches.
