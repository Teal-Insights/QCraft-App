...
**Forty-nine points of GDP is a large number to take on trust, and nothing about it is obvious.** Both runs load the same Kenyan data. Both start from the same debt stock. Both use the same equation. One of them assumes the world warms along a hot path, and seventy years later the debt ratio is nearly double.
...
**One percentage point of growth, held for three years, is worth 1.8 points of debt ratio.** Run the identical three years with growth at 7 percent instead of 6 and the ratio lands at 64.7 rather than 66.5. Held for seventy, that point is worth a different fiscal policy. That sentence is the entire reason a climate model that only touches growth can move a debt path this far.
...
**An equation that needs $r$, $g$ and $pb$ is useless until something produces them for 2030, and 2031, and every year to 2099.** That is the bulk of Q-CRAFT, and it is three separate small machines. This step takes them in the order the equation reads: growth, then the primary balance, then the interest rate.
...
**The handover year is your country's own.** The handover falls at the last year the release actually reports the figures this country's projection needs, and never later than 2029. For almost every country that is 2029, so the model takes over in 2030. For a few the release stopped reporting them much earlier: on the October 2024 data Syria's last reported year is 2010, so nearly the whole of its chart is projection. The shaded band is drawn at the country's own year rather than at a fixed 2029, and where the release ran on past a country's last reported figures the Explorer puts a notice above the results naming the year it anchored on. Check that year before you quote a date.
...
**The wedge equals the debt-stabilizing balance, because the fiscal rule is on.** Step 1 already told you how to check. Kenya's debt-stabilizing primary balance in 2050 is 0.59 percent of GDP. The two are the same number to two decimal places, which is not a coincidence: the fiscal rule is on, and holding debt at the target is exactly what the rule is doing.

**Two dials sit on the wedge**, and both of them appear again in Step 3.

**The fiscal rule decides whether the primary balance is left where the spending rule put it or pushed toward whatever holds debt at your target.** It works by adjusting next year's primary expenditure by this year's fiscal gap, which is the distance between the actual primary balance and the debt-stabilizing one from Step 1. With the rule on, Kenya's baseline converges toward the 50 percent target and holds near it, ending at 51. With it off, the ratio goes wherever the spending rule takes it.
...
**For Kenya the three rules give an effective rate in 2099 of 7.6, 2.1 and 4.5 percent.** All three baselines end at 51 percent of GDP, because the fiscal rule is doing the work in each of them. Under the Hot scenario, where no rule intervenes, they end at 100, 69 and 77.
...
**Two of the three ingredients are country-specific, and the third is not.** The distinction is worth being exact about.
...
**Most of the 49 points comes from the rigidity assumption.** Know the number before you quote the headline.
...
**The structure buys three things and costs three, and the honest way to hold them is as a pair rather than as a caveat bolted onto a result.**
...
**That is the 49 points.** Every part of it is now something you can point at.
...
2. **Employment growth, from the UN World Population Prospects.** With productivity and inflation held identical, the only remaining term in $(1+e)(1+p)(1+i)$ is employment, which comes from the working-age population projection; the one handle anyone in the tool has on it is the demography variant, Medium, High or Low. The two countries in the chapter are Kenya at plus 1.45 percent and Thailand at minus 1.11. Nothing about either country's climate exposure, borrowing or policy enters this answer.
