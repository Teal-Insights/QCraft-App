...
You will set all five of the Explorer's controls for a country and write down why you set each one where you did. Every parameter gets the same five-part treatment: what it is, why it matters, how to set it, a predict-observe-explain run in the app, and one judgment call to defend. The rationale lines you write here become the assumption section of your export packet, and they are what survives you leaving the ministry.
:::
...
```{mermaid}
%%| fig-cap: "This module is the left of the chain: the data you load, and the assumptions you set on top of it."
%%| fig-width: 6
flowchart LR
...
Three nodes are lit. The controls you set move growth and the primary balance. The interest rate is dark because V1 does not expose it.
...
::: {.callout-warning}
## SCREENSHOT-TODO
Side-by-side Analysis tab for Uganda at rigidity 1.0 and rigidity 0.0, with the same axis limits on both, so the compression is visible rather than described. Same-axis is the point, because two auto-scaled charts will hide the effect.
:::
