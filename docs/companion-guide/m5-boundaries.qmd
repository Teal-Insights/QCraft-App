...
## Fast path
No fast path. This is the shortest module and it is the one that keeps you out of trouble. If you only read one section, read @sec-m5-conservatism (the exclusions, and why the estimate is a floor).
:::
...
**It is comparable across countries.** The same growth identity, the same debt dynamics equation and the same six scenarios run for every economy in the tool. Two countries' results can therefore be set beside each other, which is what makes the diagnosis exercise later in this module work at all.
...
**All six exclusions run the same way, so the modelled impact is a lower bound you can state in one sentence.** Every exclusion is a cost left out, so the modelled fiscal impact is a lower bound on the modelled channels rather than a central estimate of total impact. The User Guide reaches the same conclusion in its own voice: the results "do not account for the potential impacts of climate change induced natural disasters, sea-level rise risks and other environmental risks, rendering the outcomes conservative" [(p. 5)](references.qmd#ref-user-guide). One sentence follows from it: *actual fiscal impacts are likely larger than these results suggest.*
...
**In such a country the baseline flattens along zero while the climate scenarios continue downward into negative territory.** Read naively, the chart shows climate scenarios with *lower* debt than baseline, which is nonsense. What you are actually seeing is one line clipped and the others not.
...
**Flooring the climate scenarios too would compress the spread between them.** Unfloored, they keep their full range, and the interpretive burden lands on you. This module is where that burden gets paid.
:::
...
4. **Neither.** Q-CRAFT does not cost adaptation, by design [(User Guide, p. 6)](references.qmd#ref-user-guide), and the LIC-DSF does not either. This question needs a costed adaptation plan, and the honest answer is to say so rather than to produce a number from a tool that cannot see it. The economics of folding adaptation into fiscal analysis is set out in [Bellon and Massetti (2022)](references.qmd#ref-bellon-2022).
