# Know what the tool cannot tell you {#sec-m5}

::: {.callout-tip appearance="simple"}
**In this module**

You will learn what Q-CRAFT leaves out and which direction that bias runs, so you can state the caveat before a reviewer does. You will learn to read a fan chart whose baseline is clipped at zero when the climate scenarios are not. Then you will practise telling a Q-CRAFT question from a debt sustainability question, in both directions.
:::
...
You have presented the fan chart. Your minister looks at the Hot Unadapted line and asks: "So this is what climate change will cost us?"
...
```{mermaid}
%%| fig-cap: "This module is about what the climate scenarios contain, and what that licenses in the write-up."
%%| fig-width: 6
flowchart LR
...
The scenarios capture one channel: the slow effect of temperature change on productivity, and through it on growth. Six things are excluded, and every one of them runs the same direction.
...
| Sea-level rise | Not a Uganda channel, and decisive for coastal and small-island economies |
...
The direction is what makes this list usable. Every exclusion is a cost left out, so the modelled fiscal impact is a lower bound on the modelled channels rather than a central estimate of total impact. One sentence follows from it: *actual fiscal impacts are likely larger than these results suggest.*
