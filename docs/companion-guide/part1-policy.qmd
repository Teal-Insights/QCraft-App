...
Q-CRAFT Explorer puts the same proven economics on a modern platform. The calculation engine is a standalone Python package with extensive automated testing. The web interface loads current data automatically, provides guidance at point of need, and makes every assumption visible and adjustable. The code is open source (MIT license) — anyone can examine, verify, and extend it.
...
Q-CRAFT projects long-term fiscal outcomes under different climate scenarios. It connects seven analytical modules, each feeding into a central debt dynamics equation.
...
The core of Q-CRAFT is the debt accumulation equation. In plain terms: next year's debt equals this year's debt, adjusted for interest payments and economic growth, minus the primary balance (revenue minus non-interest spending), plus any climate-related fiscal impact.

$$d_t = d_{t-1} \times \frac{1 + r_t}{1 + g_t} - pb_t + \text{climate}_t$$
...
| $\text{climate}_t$ | Fiscal impact of climate shocks on the debt ratio |

The intuition: when interest rates exceed growth ($r > g$), the debt ratio rises automatically — the government must run a primary surplus just to keep debt stable. When growth exceeds interest rates ($g > r$), debt dynamics are favorable and the ratio declines even with modest deficits. Climate change worsens debt dynamics by reducing growth and increasing fiscal pressures simultaneously.
...
**Baseline GDP** combines the previous three modules. Nominal GDP growth is decomposed into employment growth, productivity growth, and inflation. During the WEO forecast period (through 2028), Q-CRAFT uses the IMF's own projections. Beyond that horizon, the model takes over using the assumptions set in the dashboard.
...
**Fiscal projections** compute revenue, expenditure, and the primary balance. Revenue grows with nominal GDP (maintaining a constant revenue-to-GDP ratio). Primary expenditure, the component excluding interest payments, grows with productivity, inflation, and total population. When enabled, a fiscal rule adjusts spending to steer debt toward a target ratio.

**Climate scenarios** apply empirical estimates of how temperature changes affect GDP. Q-CRAFT uses the FADCP Climate Dataset (Centorrino, Massetti, and Tagklis, 2024), building on the Kahn et al. (2021) methodology for long-term macroeconomic effects of climate change. Six scenarios span the range from Paris-aligned (below 2°C warming) to hot-unadapted (high warming with slow policy response). Climate impacts flow through the productivity channel — reduced GDP growth means lower revenue, higher expenditure-to-GDP ratios, and accelerating debt.

::: {.callout-note collapse="true"}
## The six climate scenarios

| Scenario | Warming | Based on | Description |
|----------|---------|----------|-------------|
| **Paris** | <2°C | SSP1-2.6 | International commitments met; significant emission cuts |
| **Moderate** | ~2.5°C | SSP2-4.5 | Current trends continue; no new aggressive mitigation |
| **High** | ~3-4°C | SSP3-7.0 | Countries scale back mitigation; fragmented response |
| **Hot** | ~3-4°C (90th pctile) | SSP3-7.0 | Same emissions as High but using the 90th percentile of temperature increases |
| **Hot Adapted** | ~3-4°C | SSP3-7.0 | Same temperatures as Hot; countries adapt more quickly |
| **Hot Unadapted** | ~3-4°C | SSP3-7.0 | Same temperatures as Hot; countries adapt very slowly |

These scenarios are conservative. They do not account for climate-induced natural disasters, sea-level rise, tipping points, non-market damages (morbidity, mortality, conflict, food insecurity), or spillover effects. Actual fiscal impacts are likely larger.
:::

## How the Explorer works

Select a country from the dropdown. Q-CRAFT Explorer loads the latest available macroeconomic data from the IMF World Economic Outlook (currently April 2024) and UN population projections (2022 revision) automatically. No manual data entry needed.

Set a few parameters in the sidebar. The key choices are the demography variant (medium, high, or low population projection), productivity growth assumptions (starting rate and long-run convergence), inflation targets, interest rate approach, and whether to apply a fiscal rule with a debt target. Each parameter includes guidance linking to this companion guide. Default values provide a reasonable starting point.
...
**Completed:** Uganda (golden master country) with full baseline parity across all fiscal indicators. Smoke testing with the United States and Maldives (representing large economy and small island developing state).

**In progress:** Expanding to 14 countries spanning low-income, emerging market, and advanced economies — including Japan, Germany, Brazil, India, Nigeria, Kenya, South Africa, Fiji, Bangladesh, Mozambique, and Argentina. Testing input sensitivity across different parameter combinations.

**Next steps:** Full LIC-DSF portfolio coverage (~70 countries) and climate scenario verification.
