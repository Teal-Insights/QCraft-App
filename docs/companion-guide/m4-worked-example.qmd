...
**Initial debt level.** The published baseline puts 2023 at 47.1 percent of GDP, and Uganda's Fiscal Risk Statement reports 46.9 percent at June 2023 on the ministry's own basis. Those agree. If your Explorer run shows something materially different, the likely cause is the WEO vintage: Current mode runs WEO April 2026, Verified mode runs the workbook's own October 2024, and the published table came from the 2023 run. Note the difference rather than fixing it by overriding data.
...
**Fiscal rule convergence.** With the rule on, debt should converge toward your target rather than running away from it. Uganda's does, from below. Note in your packet that convergence from below means the rule is not binding on this path, so the target is not doing the work the trajectory might suggest it is.
...
**The task.** Compare **Hot + Adapted** against **Hot + Unadapted** for a country in the dropdown whose interest-growth differential runs against it. Temperatures are the same in both scenarios, and the difference is how quickly the economy adjusts.
