...
::: {.callout-important}
## DRAFT FOR TEAL: the headline run cannot be the rule-off run

The rule-off setting in the table above does not survive contact with the tool, and
the screenshots in this module are currently taken with the rule ON. One of the two
has to move before Tuesday.

Run Uganda on the frozen vintage with the fiscal rule off and the baseline debt ratio
falls to zero by 2050 and stays there, because the baseline applies a floor at zero.
The climate scenarios carry no floor, so they keep going: by 2099 they land between
-474 and -523 percent of GDP. The ordering inverts with them. Paris-Aligned finishes
lowest at -523 and Hot Unadapted highest at -474, so the chart says the hottest future
is the best fiscal outcome. That is the asymmetry @sec-m5 already documents, at a scale
that makes the Analysis tab unusable as teaching material. The Explorer draws those
values without the sub-zero note attached, which is a separate defect reported to the
app lane.

With the rule on, the same run reads the way this module needs: baseline 47.0 percent
of GDP in 2099, Hot Unadapted 126.8, a gap of 80 points, and the six scenarios in the
order a reader expects.

Two things follow, and they point the same way. @sec-m5 states that the zero-floor
asymmetry "is not a problem in the worked case, whose published baseline lands at 47.5
percent of GDP." That sentence is true of the rule-on run and false of the rule-off
run, so the two modules already disagree. And the rationale in the table, disclosing
the risk without assuming the policy response, is a real argument that a floored
baseline does not actually serve: a baseline pinned at zero assumes more, not less.

**The options.** Make the rule-on run the headline and report the rule-off run as the
sensitivity, which is a one-cell edit to the table above and matches the shipped
screenshots. Or keep rule-off as the headline, in which case @sec-m5 needs rewriting,
the exercise needs a floor caveat, and the screenshots have to be re-cut from the
rule-off captures that are already built and sitting beside them in
`figures/screenshots/`.

Recommendation: rule on for the headline, rule off as the named sensitivity.
:::
