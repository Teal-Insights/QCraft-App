...
| **3. Fiscal effects** | Primary and overall balance by scenario, debt-to-GDP by scenario, read against the fiscal anchor | Analysis tab for the debt chart; the exported results, CSV or workbook, for the balance series |
...
**Every column is a share of GDP or a rate, in percent, and both files carry the run manifest below the data.** The grid runs from 2009 through 2099: the WEO record and forecast, then the projection.

**The Export tab then assembles the packet proper.** One zip holds a read-me, the scenario report, an Excel workbook, the chart pack, the results CSV, the standalone charts, and a run file that restores the whole configuration on import. The assumptions annex prints all ten parameters with the rationale lines you wrote in the sidebar, and where you wrote none it says so. Add a run label and an analyst's note before you download, because both travel into every file.
