...
You will run one country the whole way through, from parameter choices to publishable prose. The worked case is Uganda, because it is the tool's golden-master verification country and every figure in it can be checked against two published documents. The scaffolding then fades: a completion problem on Ethiopia where you finish the interpretation, and an independent problem on a third country that you do alone. The target is not a chart but a write-up, in the format of the IMF's C-PIMA presentation of Q-CRAFT results and Section III of Uganda's Fiscal Risk Statement.
:::

::: {.callout-note appearance="simple"}
## Fast path
There is no fast path through this module, because it is the module the capstone is built from. If you are short of time, do @sec-m4-worked (the Uganda run, all seven steps) and @sec-m4-independent (the unscaffolded third-country problem) and skip the middle.
:::
...
Everything to the right of that gate is only as good as the baseline underneath it, which is why the sanity check comes before any climate reading.
...
**That table is the first page of your export packet.** Write it before you look at any output, so the settings are choices rather than post-hoc justifications. The five controls under Growth assumptions stay at the engine defaults for the worked case, and the packet's annex records that they were left there.

::: {.callout-note collapse="true"}
## Going deeper: why the rule is on, and what happens when it is off

Turning the fiscal rule off looks like the more neutral choice. It is not, and
the reason is worth seeing once.

Run this same case with the rule off and Uganda's baseline debt ratio falls to
zero by 2050 and stays there. The climate scenarios keep going: by 2099 they land
between -474 and -523 percent of GDP. Their order inverts on the way, so
Paris-Aligned finishes lowest and Hot + Unadapted highest, and the chart appears to
say that the hottest future is the best fiscal outcome.

Nothing has gone wrong. That is the workbook's own arithmetic, faithfully
reproduced. On the `Baseline` sheet the debt recursion is wrapped in a floor,
`=IF((...)<0,0,(...))`, so a baseline that would go negative is written as zero
instead. On the six scenario sheets the same recursion carries no floor. @sec-m5-floor
sets out the asymmetry and the reading rule for charts that touch zero. This is
what it looks like when it bites.

So a rule-off baseline is not an assumption-free baseline. It is a baseline
pinned to a floor, compared against scenarios that have none, and the comparison
between them stops meaning anything. The rule-on run is also the setting the
published workbook ships, at cell `C33` of its Dashboard, so it is the run a
reader can reproduce in Excel step for step.
...
**Debt-to-GDP trajectory (first) is the headline chart.** Follow the trajectory out of the shaded region, the WEO record and forecast, into the model's own projection. Is debt rising, stable, or falling? If the fiscal rule is on, debt should converge toward your target. If it is off, the trajectory reveals the underlying fiscal dynamics: favourable when growth exceeds interest rates, unfavourable when interest rates exceed growth.

**Revenue and Expenditure (% of GDP) (second) shows two ratios that can drift apart.** Revenue-to-GDP stays constant by assumption, because revenue grows with nominal GDP. Primary expenditure-to-GDP may diverge, because spending follows total population while revenue follows the working-age population, with productivity and inflation common to both sides. In countries where the working-age population grows faster than total population, which is the demographic dividend, expenditure-to-GDP falls and fiscal space opens. In ageing countries it rises, which is a structural fiscal pressure independent of any policy choice.

**Fiscal Balances (third) carries two lines: the primary balance, meaning revenue minus non-interest expenditure, and the overall balance, which includes interest payments.** The zero line is the reference: a primary surplus means the government is generating enough non-interest revenue to cover non-interest spending. The gap between the two lines is the interest burden on existing debt.
...
**Initial debt level: the published 47.1 percent of GDP for 2023 agrees with the ministry's own 46.9 at June 2023.** If your Explorer run shows something materially different, the likely cause is the WEO vintage: Current mode runs WEO April 2026, Verified mode runs the workbook's own October 2024, and the published table came from the 2023 run. Note the difference rather than fixing it by overriding data.

**Revenue-to-GDP: the check is whether the level is right, because the share is held constant by construction.** For a country in the middle of a revenue mobilisation strategy, a flat revenue ratio is a conservative assumption and worth one sentence in the write-up.

**Expenditure path: the published table has primary expenditure drifting from 19.9 to 19.4 percent of GDP over seventy-six years, dipping to 18.8 around 2075.** That is the demographic dividend showing up: total population growth drives spending, working-age population growth drives GDP, and for a while the second outruns the first. If your expenditure path rises instead of drifting down, check the demography variant.
...
**Balance paths: the published primary deficit narrows from 6.3 percent of GDP in 2023 to 1.4 by 2099, with the overall deficit at 3.8.** That is a large near-term consolidation built into the WEO forecast period rather than something the model invented. Whether that consolidation is credible is a question about the WEO, and it belongs in your write-up as a caveat rather than a correction.
...
**In the Briefing view, the default, the tab shows one chart: real GDP under each scenario as a percentage deviation from the baseline path, so what you read off it is the damage rather than the growth.** Switch the chart view to Workbook and the same content appears as the workbook draws it, absolute GDP levels and a GDP index set to 100 in 2029. The six scenarios run from Paris-Aligned, below 2°C warming by 2100, to Hot + Unadapted, high warming with slow adaptation. Impacts begin in 2030, and before that year every scenario sits on the baseline.

**The size of the modelled loss is country-specific even though the response rate is pooled.** The empirical estimates reflect historical relationships between temperature deviations and economic output, applied to each country's own projected warming path and its own historical norm (Step 3b of @sec-m2), so the same scenario hits two economies differently. The Uganda assessment is an example: the C-PIMA summary describes Uganda's impact as milder than other sub-Saharan African countries [(IMF, 2024)](references.qmd#src-imf-2024-uganda).

**A country showing a large gap between Paris and Hot + Unadapted faces high climate-fiscal vulnerability.** A small gap suggests lower direct climate exposure in the model, though Q-CRAFT's conservative methodology leaves out the indirect effects: natural disasters, sea-level rise, and spillover effects such as trade disruptions.
...
**If baseline debt holds stable or falls while the Hot + Unadapted path rises rapidly, climate change could destabilize an otherwise sustainable fiscal position.** That is the core finding Q-CRAFT is designed to surface, and it answers one question: even if our fiscal policy is sound today, could climate change make it unsustainable?
...
- **Download all scenarios (CSV):** baseline plus all six climate scenarios, stacked with a scenario column. Use this for custom analysis, cross-country comparison, or integration with other tools.
...
**The Export tab then assembles the packet proper.** One zip holds a read-me, the scenario report, an Excel file, the chart pack, the results CSV, the standalone charts, and a run file that restores the whole configuration on import. The assumptions annex prints all ten parameters with the rationale lines you wrote in the sidebar, and where you wrote none it says so. Add a run label and an analyst's note before you download, because both travel into every file.
...
**You run a new country with the same method, and most of the setup is fixed for you.** Ethiopia earns its place here because its demography runs the other way from most countries in a fiscal risk conversation: the working-age population is still climbing, which pushes the baseline in one direction while climate damage pushes the other.
...
**Compare Hot + Adapted against Hot + Unadapted for a country in the dropdown whose interest-growth differential runs against it.** Temperatures are the same in both scenarios, and the difference is how quickly the economy adjusts.
