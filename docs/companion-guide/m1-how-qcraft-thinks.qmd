...
1. **Up-to-date data.** A posted workbook ships with the WEO vintage it was built on, and that vintage ages between releases. The Explorer ships two data modes: Current, the default, carries the latest bundled release, WEO April 2026 as this is written, and Verified carries the workbook's own WEO October 2024 vintage so the tool can be checked against the original. Refreshing the data is a release rather than a download and a rebuild.
...
**You now have a complete long-term fiscal projection model: two published data sources, three manufactured numbers, one equation, one debt path.** Every sidebar control except expenditure rigidity works on this machine; rigidity waits for the warming block. It works for any country in the tool, and nothing in it knows that the climate exists.
...
Four of the five land on two of the three numbers. Most questions that arrive as five separate problems are two.
