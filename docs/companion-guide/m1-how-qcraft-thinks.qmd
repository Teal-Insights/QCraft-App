...
Q-CRAFT Explorer puts that same economics on the web. Four things are different, and they are the reasons it exists.

**Data currency.** A posted workbook ships with the WEO vintage it was built on, and that vintage ages between releases. The Explorer carries current bundled data, October 2024 as this is written, and refreshing it is a release rather than a download and a rebuild.

**Guidance where you need it.** The help for choosing a parameter sits next to the parameter. That is the difference between reading about expenditure rigidity and choosing a value for it while looking at what your own country's spending has actually done, which is what @sec-m3 has you do.

**Fewer steps to the analysis.** Less of the hour goes on mechanics, so more of it goes on the judgment calls that are the actual work.

**The export packet.** Your settings, your written reason for each one, and presentation-ready output come out together, so the record of what you decided travels with the numbers. @sec-m3 is where you build one, and @sec-m6 is where you hand it over.
...
This course teaches the Explorer only. The stated intent is to teach Q-CRAFT both ways: where to download the workbook, how its sheets map onto the three numbers below, and how to run the worked case in @sec-m4 there as well as here. That material does not exist yet. Scope decision for review.
:::
...
::: {.callout-important}
## DRAFT FOR TEAL: the kitchen version, if you want one

Optional, and skippable. Some readers hold the chain more easily as a kitchen.
...
**Where the analogy breaks.** A cook tastes and adjusts, and this model does not. Nothing downstream feeds back into the ingredients, so cutting spending inside the model never slows growth, which is the partial-equilibrium limit stated above. And a recipe is a claim about what will come out of the oven. A projection to 2099 is not. It is a comparison between two dishes made from deliberately different seasoning.
:::
