# Uganda end to end: from assumptions to the Fiscal Risk Statement paragraph {#sec-m4}

::: {.callout-tip appearance="simple"}
**In this module**

You will run one country the whole way through, from parameter choices to publishable prose. The target is not a chart but a write-up, in the format of the IMF's C-PIMA presentation of Q-CRAFT results and Section III of Uganda's own Fiscal Risk Statement. The scaffolding fades as you go: a full worked case first, then a variant where you finish the interpretation, then one you do alone.
:::
...
**The IMF's C-PIMA write-up.** *Uganda: PFM Climate Assessment: Public Investment and Fiscal Risks Management* (IMF, 2024) reports the September 2023 workshop with MoFPED staff in a single paragraph. Climate change looks milder in Uganda than in other sub-Saharan African countries, it says, and it still matters for growth and fiscal sustainability, because by end of century GDP loss could pass 4 percentage points with debt at 66 percent of GDP against 47.5 percent in the baseline. One paragraph. Three numbers. A comparative judgment.
...
```{mermaid}
%%| fig-cap: "This module is the output end: debt paths, the packet, and the prose that comes out of them."
%%| fig-width: 6
flowchart LR
    ...
    style O fill:#1ABC9C,stroke:#2C3E50,color:#fff
...
Section III of Uganda's Fiscal Risk Statement FY 2024/25 is organised in three moves. Copy that structure, because it is the one the ministry already publishes.
...
## Completion problem: the High scenario variant {#sec-m4-completion}

Same country, same parameters, one change: report against the **High** scenario rather than Hot. The first steps are done for you, and you finish the interpretation.
...
1. Read the 2099 debt ratio under High from the Analysis tab. Record it. ____
2. Compute the gap to baseline. ____ percentage points.
3. Read the year the High path crosses 50 percent of GDP, if it does. ____
...
## Independent problem: the adaptation-speed variant {#sec-m4-independent}

No scaffolding this time, and a checklist only.

**The task.** Compare **Hot + Adapted** against **Hot + Unadapted** for Uganda. Temperatures are the same in both, and the difference is how quickly the economy adjusts.
