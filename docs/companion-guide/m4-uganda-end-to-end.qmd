...
## Fast path
There is no fast path through this module. It is the module the capstone is built from. If you are short of time, do @sec-m4-worked and @sec-m4-independent and skip the middle.
:::
...
## The hook: two documents that already exist
...
| Expenditure rigidity | 1.0 | Tool default, conservative; budget composition evidence per @sec-m3; 0.6 tested as sensitivity |
...
**The shape.** Look at when the lines separate, not just where they end. Scenarios are identical until 2030, then fan slowly. A path that crosses the ceiling in 2085 and one that crosses in 2060 imply very different urgency, and the fan chart shows the difference at a glance where a 2099 table does not.
...
::: {.callout-note collapse="true"}
## The trap, if you want to check before you write
