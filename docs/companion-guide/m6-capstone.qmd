...
**The packet puts your assumptions, your rationales and presentation-ready output into one handover, the fourth reason from @sec-m1 in its finished form.** That handover frees the week for the policy question rather than for assembling the evidence by hand.

**Pick the country you work on, because the capstone is a rehearsal for the document you will have to write.** Pick Uganda instead if you want a run you can compare against the published section @sec-m4 works through. Avoid Ethiopia and whichever country you took for the independent problem in @sec-m4, since you have already worked those. A small number of countries do not project, because the published record is missing a figure the projection has to start from. The Explorer refuses those with a notice rather than inventing the number. If yours is refused, record the notice and take a second country for the packet. A few others project from an earlier anchor year than the data release runs to. Where the Explorer names that year on screen, carry it into your write-up. Neither case is a fault in the run. Both are facts about what the source publishes for your country.

**Matching anyone else's published figures is not required.** You are on a later WEO vintage, possibly with different parameters, and reproducing someone else's numbers is not the skill being assessed.
...
The rubric scores four criteria, and the weights are a proposal rather than a decision.
...
**Under the proposed weights, your assumptions and your baseline check settle sixty percent of the mark, and both happen before anyone reads a number you produced.**
...
**If your A or B answer moved up, the course worked.** If you have completed the capstone, C3 is true by construction, because you have run the tool and written the explanation. If C3 is true but A and B did not move, you can drive the tool while the economics underneath it did not land. That is worth a conversation rather than a shrug.
...
**Two follow-ups are designed into the course, to ship with the workshop materials (see the TODO below).**
