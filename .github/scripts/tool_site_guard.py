...
def check(env, main_sha, runs):
    if env["GITHUB_REF"] != "refs/heads/main" or main_sha != env["GITHUB_SHA"]:
        raise ValueError(
            "Dispatch must use unchanged main at its exact workflow commit."
        )
    if env.get("GITHUB_RUN_ATTEMPT", "1") != "1":
        raise ValueError(
            "A rerun is not the unique dispatch recorded for this release."
        )
    ...
    own = own[0]
    expected_title = (
        f"Tool release: {env['OPERATION']} | {env['DISPATCH_RECEIPT_SHA256']}"
    )
    if (
        own["head_sha"] != env["GITHUB_SHA"]
        or own["event"] != "workflow_dispatch"
        or own["path"].split("@")[0] != PATH
        or own["display_title"] != expected_title
    ):
    ...
    if env["OPERATION"] == "restore-prior-site":
        ...
        if (
            ...
            raise ValueError(
                "Restore target must be the exact completed forward dispatch."
            )
    ...
    for run in runs:
        ...
        if run["status"] != "completed" or stamp(run["created_at"]) >= boundary:
            raise ValueError(
                f"Other active, same-time or newer dispatch blocks operation: "
                f"{run['id']}."
            )
    return {
        "permitted_by_state_guard": True,
        "operation": env["OPERATION"],
        "dispatch_run_id": own["id"],
        "workflow_main_sha": main_sha,
...
def main():
    ...
    def api(route, *args):
        return json.loads(
            subprocess.check_output(["gh", "api", *args, route], text=True)
        )

    main_sha = api(f"repos/{repo}/git/ref/heads/main")["object"]["sha"]
    pages = api(
        f"repos/{repo}/actions/workflows/companion-guide.yml/runs?event=workflow_dispatch&per_page=100",
        "--paginate",
        "--slurp",
    )
    ...
    if api(f"repos/{repo}/git/ref/heads/main")["object"]["sha"] != main_sha:
        raise ValueError("Main changed during the paginated inventory read.")
