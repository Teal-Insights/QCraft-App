...
**Module boundary (definitive):** The WEO overlap productivity derivation belongs EXCLUSIVELY in `baseline_v1()` (SPEC Section 4.4, Phase 2). The `productivity_country()` module returns only the pure logistic convergence curve. It does NOT perform the WEO back-calculation. Any values for years [WEO_MAX_YEAR-6, WEO_MAX_YEAR] returned by `productivity_country()` are placeholders that `baseline_v1()` will overwrite. The golden master intermediate CSV for productivity contains WEO-derived values because it captures the combined output after baseline_v1's overwrite, not the raw productivity module output. The intermediate golden master tests the combined output (after baseline_v1 overwrite).

### 3. Logistic convergence function (2030-2099)

**CRITICAL: The golden master proves the logistic function starts at 2030, NOT 2029.** Year 2029 shows 2.47% (WEO-derived), while 2030 shows 4.89% (logistic, near start=5.0%). The SPEC says `counter = t - WEO_MAX_YEAR` with WEO_MAX_YEAR=2028, which gives counter=1 for 2029 and counter=2 for 2030. However, logistic(counter=1) = ~4.89% and logistic(counter=2) = ~4.85%, which matches 2030 and 2031 in the golden master. The resolution is that the macrofiscal data extends through 2029, making `WEO_MAX_YEAR = max(macrofiscal.years) = 2029` at runtime. The SPEC annotation "2028" is stale. The Excel analysis documents confirm "Historical period: 2001-2029, Projected: 2030-2099." The WEO overlap productivity derivation belongs EXCLUSIVELY in `baseline_v1()` (SPEC Section 4.4, Phase 2). The `productivity_country()` module returns only the pure logistic convergence curve. It does NOT perform the WEO back-calculation. Any values for years [WEO_MAX_YEAR-6, WEO_MAX_YEAR] returned by `productivity_country()` are placeholders that `baseline_v1()` will overwrite.
