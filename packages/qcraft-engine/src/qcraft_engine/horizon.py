...
def resolve_horizon(payload):
    ...
    source_end = max(
        (y for y, r in macro.items() if any(finite(r.get(k)) for k in REQUIRED_MACRO)),
        default=0,
    )
    ...
    for year in range(YEAR_START, source_end + 1):
        ...
        if (
            not missing
            and not finite(row.get("interest_rate_percent"))
            and row["debt"] != 0
        ):
    ...
    prod = {
        r["years"]: r["productivity_level"]
        for r in payload["productivity"]
        if r["iso3c"] == iso
        and finite(r.get("productivity_level"))
        and r["productivity_level"] > 0
    }
    wdi_end = min(max(prod, default=0), horizon)
    if not unsupported and any(
        y not in prod for y in range(YEAR_START - 1, wdi_end + 1)
    ):
        reason = (
            "The WDI history needed for consecutive productivity growth is incomplete."
        )
        unsupported = True
        ...
        for scenario in CLIMATE_SCENARIOS:
            for year in range(horizon, YEAR_END + 1):
                ...
                if not finite(loss) or loss <= -100:
                    reason = (
                        f"No usable climate index for {scenario} "
                        f"at calendar year {year}."
                    )
                    unsupported = True
                    break
            if unsupported:
    ...
    return {
        ...
        if unsupported
        else ("shorter" if horizon < source_end else "full"),
